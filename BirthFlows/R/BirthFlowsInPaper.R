...
# a version of BirthFlows to be placed in situ in text.
...
pdf("Figures/BirthFlowsInText.pdf",height=5,width=10)
...
plot(NULL, 
		type = "n", 
		xlim = c(1687, 2077), # odd nr, due to recalculations
...
C20 <- as.character(Cohs[Cohs%%20 == 0])
for (i in seq(5,nrow(PC5cs)-2,by=4)){
	if (C5[i] >= 1700 & C5[i-1] != 1720){
		ind <- which(PC5[as.character(C5[i]),] > 500)[1]
		text(Yrs[ind], 
				PC5cs2[i, ind]+3e3,
...
text(1815, 9000, "crude growth", col = "#FFFFFF",cex=.8)
text(1904, -10000, "contraction", col = "#FFFFFF",cex=.8)
