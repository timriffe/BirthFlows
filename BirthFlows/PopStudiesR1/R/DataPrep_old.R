...
# saveRDS(masfr, here("Data","ReduxTest","masfr_old.rds"))
...
#saveRDS(B_1736to1750, here("Data","ReduxTest","B_1736to1750_old.rds"))
# ----------------------------------------------#
# Now adjustments for births in years 1751-1774 #
# ----------------------------------------------#
Expos        <- readRDS(here("Data","Expos1751to1774.rds"))
...
SWEh  <- read.csv(here("Data","SWEbirths.txt"), na.strings = ".", stringsAsFactors = FALSE)
...
saveRDS(SWE, file = here("Data","SWE_sm_old.rds"))
...
asfr          <- read.table(here("Data","forecast","SWEasfrRR.txt"), 
...
PProj           <- read.table(here("Data","forecast","scb-females-projected-swe.csv"),
		             skip = 0,
					 as.is = TRUE, 
					 header = TRUE)
...
SWE$ARDY <- SWE$Year - SWE$Cohort 
saveRDS(SWE, file = here("Data","SWE_final_old.rds"))
...
Lineage                      <- read.csv(here("Data","Swedishfamilybranch.csv"))
...
L1 <- Lineage
# another one to try
Lineage                      <- read.csv(here("Data","Swedishfamilybranch2.csv"))
Lineage$year.daughters.birth <- c(Lineage$date.of.birth[-1],NA)
